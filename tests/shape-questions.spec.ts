...
test('merging and reloading keeps all three skills and their paused reviews independent', async ({ page }) => {
  await page.clock.setFixedTime(new Date('2026-09-07T12:00:00Z'));
  await page.goto('/');
  const session = await page.evaluateHandle(async () => {
    const sessionPath = '/src/session.ts';
    const progressPath = '/src/progress.ts';
    // Static imports run in Node; these sessions need browser storage and its fixed clock.
    ...
    // Each device shares one answer, then practices a different recognition skill.
    const original = new LearnerSession(null);
    original.start();
    original.answer(-52, -12);
    const branch = (skill: 'location-to-name-recognition' | 'shape-recognition') => {
      const learner = new LearnerSession(null, original.snapshot());
      learner.choosePractice({
        scope: 'countries', continent: 'Worldwide', region: 'All regions', learning: skill,
      });
      learner.answerCountry(learner.searchCountries('bRÁz')[0].properties.id);
      const practice = learner.snapshot();
      practice.current = { countryId: 'BRA', skill, kind: 'practice', assisted: false };
      practice.cursor = practice.attempts.length;
      learner.replace(practice);
      learner.answerCountry('ARG');
      learner.retry();
  ...
  const restored = await session.evaluate(({ learner, expectedIds }) => ({
    ...
    current: learner.snapshot().current, paused: learner.snapshot().pausedQuestions,
    proficiency: learner.proficiency,
  }));
  expect(restored.ids).toEqual(restored.expectedIds);
  expect(restored.current).toMatchObject({ countryId: 'BRA', skill: 'shape-recognition', kind: 'retry' });
  expect(restored.paused).toEqual([
    { countryId: 'BRA', skill: 'location-to-name-recognition', kind: 'retry', assisted: false },
  ]);
  ...
  expect(await session.evaluate(({ learner }) => {
    ...
    kind: 'retry', feedback: { skill: 'shape-recognition', correct: true },
    proficiency: { level: 'Learning', dueAt: '2026-09-07T12:10:00.000Z' },
  });
  await session.dispose();
});
