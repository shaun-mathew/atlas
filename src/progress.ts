...
const attemptSchema = z.object({
  ...
  toleranceKm: z.number().finite().positive().optional(),
...
const legacyProgressSchema = z.object({
  ...
  return {
    ...initialProgress(), ...state, version: 7 as const, current,
...
function attemptContent(attempt: Omit<Attempt, 'id'>): string {
  ...
  // Append city scoring only when present: old country migration hashes and
  // immutable identity comparisons must remain byte-for-byte unchanged.
  ...
  .superRefine((state, context) => {
    ...
    for (const item of [...state.pausedQuestions, ...(state.current ? [state.current] : []), ...state.attempts]) {
      ...
      if (!city || city.countryId !== item.countryId
        ...
        context.addIssue({ code: 'custom', message: 'A city learning item must use its parent country and an eligible skill.' });
      }
    }
...
export function mergeProgress(account: Progress, incoming: Progress): Progress {
  ...
  for (const state of [saved, received]) {
    if (state.current?.assisted) exposed.add(entityKey(state.current));
    ...
    for (const question of questions) {
      ...
      if (question.assisted) exposed.add(entityKey(question));
    }
  }
  ...
  if (current) {
    ...
    current.assisted ||= exposed.has(entityKey(current));
